...
try:
    ...
    # Surface a clear error, but do not leave dead code after raise
    raise ImportError('Failed to import cvxopt. Some quality metrics will not be available.') from e
...
class PointGraspMetrics3D:
    ...
    def grasp_quality(grasp, obj, params, contacts=None, vis=False):
        ...
        soft_fingers = params.soft_fingers if params.contains("soft_fingers") else None
        check_approach = params.check_approach if params.contains("check_approach") else None
        ...
        assert contacts is not None, f"contacts: {contacts}"
        if contacts is None:
        ...
        for i in range(num_contacts):
            ...
            # contact_forces.shape = (3,8); contact_outward_normal.shape = (3,)
            # print(f'after friction cone')
    ...
    def grasp_matrix(forces, torques, normals, soft_fingers=False,
        ...
        if soft_fingers:
            # response for the extra two columns.
    ...
    def partial_closure(forces, torques, normals, soft_fingers=False,
        ...
        # Use integer division to avoid float indices in slicing
        num_wrenches_per_finger = forces.shape[1] // num_fingers
        G = np.zeros([6, 0])
        for i in range(num_fingers):
            start_i = int(num_wrenches_per_finger * i)
            end_i = int(num_wrenches_per_finger * (i + 1))
    ...
    def wrench_resistance(forces, torques, normals, soft_fingers=False,
        ...
        """
        if params is None:
            print(f'params is None and return 0')
            return 0
        force_limit = params.force_limits
        target_wrench = params.target_wrench
        if 'wrench_norm_thresh' in list(params.keys()):
        ...
        # print('I am in wrench resistance')
        # from IPython import embed; embed()
        
        # reorganize the grasp matrix for easier constraint enforcement in optimization
        num_fingers = normals.shape[1]
        # Use integer division to compute per-finger wrench count
        num_wrenches_per_finger = forces.shape[1] // num_fingers
        G = np.zeros([6, 0])
        for i in range(num_fingers):
            start_i = int(num_wrenches_per_finger * i)
            ...
            G_i = PointGraspMetrics3D.grasp_matrix(forces[:, start_i:end_i], torques[:, start_i:end_i],
                                                   ...
                                                   soft_fingers, params=params)
            # from IPython import embed; embed()
            G = np.c_[G, G_i] # concat each finger's [force torque*\alpha;]
        ...
        if wrench_resisted:
            ...
            logging.debug(f"{1.0/(2*force_limit)} substraced because of force_limit.")
        return Q
    ...
    def wrench_in_positive_span(wrench_basis, target_wrench, force_limit, num_fingers=1,
        ...
        """
        # logging.debug(wrench_basis.shape)
        # logging.debug(target_wrench.shape)
        num_wrenches = wrench_basis.shape[1]

        # Coerce target_wrench to 6x1 column for cvxopt and consistent math
        target_wrench = np.asarray(target_wrench, dtype=float)
        if target_wrench.ndim == 1:
            target_wrench = target_wrench.reshape((-1, 1))
        if target_wrench.shape != (6, 1):
            raise ValueError('target_wrench must have shape (6, 1); got %s' % (target_wrench.shape,))
        ...
        # Compare wrenches as flat vectors
        min_dist = np.linalg.norm(wrench_basis.dot(v).ravel() - target_wrench.ravel()) ** 2
        # print('I am in wrench in positive span')
        # from IPython import embed; embed()
        # add back in the target wrench
        # print(f"\033[31m min_dist: {min_dist}, thresh: {wrench_norm_thresh} \033[0m")
