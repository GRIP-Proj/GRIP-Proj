...
if __name__ == "__main__":
    ...
    grip_head_dataset = GRIP_backbone_dataset(
        dataset_path='/home/diligent/Desktop/semantic_grasp_DiT/dataset_grasp/torch_tensor/graspGen/train',
        dtype=dtype,
        device='cpu',
        mmap=args.mmap
    )
    grip_head_val_dataset = GRIP_backbone_dataset(
        dataset_path='/home/diligent/Desktop/semantic_grasp_DiT/dataset_grasp/torch_tensor/graspGen/val',
        dtype=dtype,
        device='cpu',
        mmap=args.mmap
    )
    ...
    for i in tqdm(range(epochs)):
        ...
        for batch_idx, (x, y, pd) in enumerate(train_dataset):
            x = x.to(args.device, dtype)
            y = y.to(args.device, dtype)
            pd = pd.to(args.device, dtype)

            optimizer.zero_grad()
            loss = grip.backbone_forward(x, y, pd)
            loss.backward()
            optimizer.step()
        ...
        val_loss = 0.0
        val_loss_item = 0
        for batch_idx, (x, y, pd) in enumerate(val_dataset):
            x = x.to(args.device, dtype)
            y = y.to(args.device, dtype)
            pd = pd.to(args.device, dtype)
