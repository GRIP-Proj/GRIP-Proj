...
# model_path = './save_ckpt/epoch0001000.pt'
model_path = './model_ckpt/ckpt_RDT_b64_lr1e-4_DDP/epoch0001000.pt'
...
for uuid in os.listdir(graspGen_dataset_path):
    ...
    for key in grasp.keys():
        ...
        model_kwargs = dict(c=text_embedding, pd=point_feat)
        ...
        transform[:3, 2] = np.cross(transform[:3, 0], transform[:3, 1])
        transform[:3, 3] = result[0, 6:]
        ...
        # print(f'object center is {np.mean(object_pd[:, :3], axis=0)}')
        # print(f'grasp contact point is {transform[:3, 3]}')
        
        
        o3d.visualization.draw_geometries([pcd, contact_point, gripper])
        break
