...
class RDTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, **block_kwargs):
        ...
        self.mlp_down = nn.Sequential(
            ...
            nn.ReLU(),
            nn.LayerNorm(hidden_size),
            nn.Linear(hidden_size, 18, bias=True)
        )
        self.text_down = nn.Sequential(
            nn.Linear(512, hidden_size*2, bias=True),
            nn.ReLU(),
            nn.LayerNorm(hidden_size*2),
            nn.Linear(hidden_size*2, hidden_size, bias=True),
            nn.ReLU(),
        ...
        self.cross_attn_text_pd = CrossAttention(
            hidden_size, num_heads=num_heads, 
            qkv_bias=True, qk_norm=True, 
            norm_layer=RmsNorm,**block_kwargs)
    ...
    def forward(self, x, t, c, pd, mask=None):
        ...
        x = self.mlp_down(x) # down-project to 9-dim
