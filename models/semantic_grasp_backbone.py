...
class Dataset_SemanticGrasp(Dataset):
    def __init__(self, x, y, pd, transform=None):
        self.x = x
        self.y = y
        self.pd = pd

    def __len__(self):
        return len(self.x)     # 数据总数
...
class PointMLP(nn.Module):
    def __init__(self, in_dim=64, hidden_dim=1024, out_dim=512, num_point=1000):
        super().__init__()

        self.mlp = nn.Sequential(
            nn.Linear(in_dim*num_point, hidden_dim),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        # x = x.mean(dim=1)  # (B, 512)  -> average pooling over points
        return x

class Semantic_Grasp(nn.Module):
