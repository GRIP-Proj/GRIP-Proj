...
grasp_pose_path = '/home/diligent/Desktop/semantic_grasp_DiT/dataset_grasp/selected_grasp_dataset/gpd'
...
for train_type in type_list:
    ...
    for uuid in os.listdir(join(grasp_folder)):
        ...
        for item in json_data:
            ...
            for grasp_key in grasps.keys():
                contact_point_l = np.array(grasps[grasp_key]['final_contact_point_l'])
                contact_point_r = np.array(grasps[grasp_key]['final_contact_point_r'])
                
                
                if contact_point_l is None or contact_point_r is None:
                    continue
                try:
    ...
    print(f'Object type: {train_type} - Average Wrench Resistance Score: {np.mean(np.array(score_list))} over {grasp_num} grasps')
    with open("output_gpd.txt", "w") as f:
